...
HISTORICAL_TARGET_RISK_FALLBACK_USD = 37.5
...
def save_settings(total_deposited, risk_pct_input, current_nav=None):
    payload = load_settings()
    payload["total_deposited"] = total_deposited
    payload["risk_pct_input"] = risk_pct_input
    if current_nav is not None:
        payload["current_nav"] = current_nav
...
def load_data():
    ...
    if not df.empty:
        cols = ['image_url', 'notes', 'management_notes', 'setup_type', 'quality', 'score', 'symbol', 'side', 'pnl_usd', 'trade_date', 'stop_loss', 'initial_stop', 'price', 'quantity', 'campaign_id', 'management_state', 'target_risk_usd']
        for col in cols:
            if col not in df.columns: df[col] = None

        df['setup_type'] = df['setup_type'].fillna("Uncategorized")
        df['trade_date'] = pd.to_datetime(df['trade_date'])
        for num_col in ['pnl_usd', 'price', 'quantity', 'stop_loss', 'initial_stop', 'quality', 'score', 'target_risk_usd']:
...
if regime['ok']:
    ...
    for basis_line in rd.get('basis', [])[:3]:
        st.sidebar.caption("• " + str(basis_line))
else:
...
gov_res = ec.compute_risk_governor(raw_df, regime, current_acc_size, risk_pct_input)
st.sidebar.markdown("---")
st.sidebar.subheader("🧭 Risk Governor")
if gov_res["ok"]:
    gd = gov_res["data"]
    gov_msg = f"**{gd['trade_mode']}** | Risk: **{gd['allowed_risk_pct']:.2f}%** | Exposure: **{gd['allowed_exposure_pct']:.0f}%**"
    if gd["trade_mode"] in ["BLOCKED", "PILOT"]:
        st.sidebar.error(gov_msg)
    elif gd["trade_mode"] == "REDUCED":
        st.sidebar.warning(gov_msg)
    else:
        st.sidebar.success(gov_msg)
    st.sidebar.caption(
        f"Expectancy20: {gd['rolling_expectancy_20']:.2f}R | "
        f"Loss streak: {gd['loss_streak']} | "
        f"DD20 now: {gd['personal_drawdown_r']:.1f}R | Max20: {gd.get('recent_max_drawdown_r', 0):.1f}R | Open*: {gd.get('open_portfolio_r_weighted', 0):.1f}R"
    )
    st.sidebar.caption(gd["reason"])
else:
    st.sidebar.warning(f"Risk Governor unavailable: {gov_res.get('error')}")

exp_gov_res = ec.compute_exposure_governor(raw_df, regime, gov_res, current_acc_size)
st.sidebar.markdown("---")
st.sidebar.subheader("📡 Exposure Governor")
if exp_gov_res["ok"]:
    ed = exp_gov_res["data"]
    exp_msg = "{} | {:.1f}% / {:.0f}%".format(ed["status"], float(ed["total_exposure_pct"]), float(ed["allowed_exposure_pct"]))
    if ed["severity"] == "red":
        st.sidebar.error(exp_msg)
    elif ed["severity"] == "yellow":
        st.sidebar.warning(exp_msg)
    elif ed["severity"] == "blue":
        st.sidebar.info(exp_msg)
    else:
        st.sidebar.success(exp_msg)
    st.sidebar.caption("Utilization: {:.0f}% | Cash: {:.1f}% | ALGO: {:.1f}%".format(float(ed["utilization_pct"]), float(ed["cash_pct"]), float(ed["algo_exposure_pct"])))
    st.sidebar.caption(ed["action"])
else:
    st.sidebar.warning("Exposure Governor unavailable: {}".format(exp_gov_res.get("error")))
...
def compute_live_portfolio_data(open_trades_dict, _acc_size, _target_risk_usd, _spy_hist):
    ...
    for row in open_trades_dict:
        ...
        effective_target_risk = hist_target_risk if hist_target_risk > 0 else _target_risk_usd

        open_r_val = (open_pnl / effective_target_risk) if str(setup).upper() == 'ALGO' and effective_target_risk > 0 else (open_pnl / original_campaign_risk if original_campaign_risk > 0 else 0)
        total_pnl = open_pnl + row['realized_pnl']
        total_campaign_r = (total_pnl / effective_target_risk) if str(setup).upper() == 'ALGO' and effective_target_risk > 0 else (total_pnl / original_campaign_risk if original_campaign_risk > 0 else 0)
        
        eval_res = ec.evaluate_position_engine(
            sym, entry, row['entry_date'], sl, setup, row['management_state'], 
            weight_pct, total_campaign_r, target_risk_usd=effective_target_risk, actual_risk_usd=original_campaign_risk, spy_hist=_spy_hist, quantity=qty, initial_quantity=base_qty
        )
...
if df.empty:
    ...
    if 'campaign_id' in df_sorted.columns:
        for cid, group in df_sorted[df_sorted['campaign_id'].notnull()].groupby('campaign_id'):
            ...
            risk_vals = pd.to_numeric(group["target_risk_usd"], errors="coerce") if "target_risk_usd" in group.columns else pd.Series(dtype=float)
            risk_vals = risk_vals[(risk_vals > 0) & risk_vals.notna()]
            effective_target_risk = float(risk_vals.iloc[0]) if not risk_vals.empty else HISTORICAL_TARGET_RISK_FALLBACK_USD

            r_realized = (total_pnl / effective_target_risk) if is_algo and effective_target_risk > 0 else ((total_pnl / original_campaign_risk) if original_campaign_risk > 0 else 0)
...
# Sentinel Truth Center routes.
try:
    for _sentinel_dashboard_obj_name in ("app", "server", "dash_app"):
        _sentinel_dashboard_obj = globals().get(_sentinel_dashboard_obj_name)
        if _sentinel_dashboard_obj is not None:
            if dashboard_truth_center.register_routes(_sentinel_dashboard_obj):
                break
except Exception as _truth_center_error:
    print(f"TruthCenter warning: {_truth_center_error}")
