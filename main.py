...
def get_alert_action_keyboard(task_id):
    return {
        "inline_keyboard": [[
            {"text": "✅ בוצע", "callback_data": f"at_done|{task_id}"},
            {"text": "📝 לא בוצע", "callback_data": f"at_no|{task_id}"}
        ]]
    }

def concrete_partial_text(qty, pct=25):
    try:
        q = float(qty or 0)
    except Exception:
        q = 0
    if q <= 0:
        return "חסרה כמות פתוחה; לא שולח מימוש חלקי אוטומטי"
    if q < 2:
        return "כמות 1: לא לממש חלקית; לנהל דרך קידום סטופ"
    sell_qty = max(1, round(q * (pct / 100)))
    if sell_qty >= q:
        sell_qty = max(1, q - 1)
    actual_pct = (sell_qty / q) * 100 if q else 0
    return f"לממש {sell_qty:g} מתוך {q:g} מניות (~{actual_pct:.0f}% בפועל)"

def send_action_alert(trade_id, campaign_id, symbol, alert_type, text, meta=None):
    task_id = at.create_task(trade_id, campaign_id, symbol, alert_type, text, meta or {})
    send_telegram_msg(text + at.action_footer(), keyboard=get_alert_action_keyboard(task_id))
...
def get_current_risk_snapshot():
    try:
        ...
        nav = float(settings.get("current_nav") or settings.get("total_deposited") or 7500.0)
        risk_pct = float(settings.get("risk_pct_input", 0.5))
        return {
            "target_risk_usd": nav * (risk_pct / 100),
            "account_nav_snapshot": nav,
            "risk_pct_snapshot": risk_pct
        }
    except Exception as e:
        log(f"⚠️ Risk snapshot fallback used: {e}")
        return {"target_risk_usd": 37.5, "account_nav_snapshot": 7500.0, "risk_pct_snapshot": 0.5}
    ...
    try:
        ...
        for attempt in range(3):
            ...
            if "<Flex" in res_data.text or "<Trade" in res_data.text:
                nav_result = navsvc.update_nav_from_xml(res_data.text, source="ibkr_flex_getstatement")
                if nav_result.get("nav"):
                    log(f"🏦 IBKR NAV auto-updated from report: ${nav_result.get('nav'):,.2f}")
                else:
                    log("⚠️ IBKR report received but NAV missing. Check Flex Query includes ChangeInNAV endingValue.")
...
def check_live_alerts():
    ...
    for _, row in df_open.iterrows():
        ...
        net_qty = float(row.get('net_qty', row.get('quantity', 0)) or 0)
        ...
        if sl > 0 and current_price <= sl and "SL_SENT" not in current_notes:
            alert_text = f"🚨 *התרעת STOP LOSS נחצתה!*\n\n▪️ נכס: `{sym}`\n▪️ מחיר נוכחי: `${current_price:.2f}`\n▪️ סטופ מוגדר: `${sl:.2f}`\n\n⚠️ חגורת הבטיחות נפרצה! המחיר ירד מתחת לסטופ. סגור את הפוזיציה מיד בברוקר."
            send_action_alert(t_id, cid, sym, "STOP_LOSS", alert_text, {"price": current_price, "stop": sl})
        ...
        # 🎯 התרעות R-Multiples מבוססות סיכון התחלתי
            ...
            if risk > 0:
                ...
                if rr >= 3.0 and "3R_SENT" not in current_notes:
                    alert_text = f"🎯 *התרעת יעד רווח 3R!*\n\n▪️ נכס: `{sym}`\n▪️ R:R צף: `{rr:.2f}R`\n\nפעולה: *{concrete_partial_text(net_qty, 25)}*. לאחר מכן לקדם סטופ לפחות לאזור כניסה/סיכון אפס."
                    send_action_alert(t_id, cid, sym, "TARGET_3R", alert_text, {"rr": rr, "price": current_price})
                    ...
                    alert_text = f"🎯 *התרעת יעד רווח 2R!*\n\n▪️ נכס: `{sym}`\n▪️ R:R צף: `{rr:.2f}R`\n\nפעולה: *לקדם סטופ לאזור כניסה / סיכון אפס: `${avg_price:.2f}`.* לא לממש חלקית אלא אם מופיעה חולשה נוספת."
                    send_action_alert(t_id, cid, sym, "TARGET_2R", alert_text, {"rr": rr, "price": current_price})
...
def sync_logic():
    ...
    for t in trades:
        ...
        if not existing:
            ...
            # --- הקפאת סיכון היסטורי לקמפיין ---
            snap = get_current_risk_snapshot()
            try:
                campaign_risk = None
                if cid and db_data:
                    for old in db_data:
                        if str(old.get("campaign_id")) == str(cid):
                            old_risk = old.get("target_risk_usd")
                            if old_risk not in [None, "", 0, "0"]:
                                campaign_risk = float(old_risk)
                                break
                clean_info["target_risk_usd"] = float(campaign_risk if campaign_risk else snap["target_risk_usd"])
                clean_info["account_nav_snapshot"] = float(snap["account_nav_snapshot"])
                clean_info["risk_pct_snapshot"] = float(snap["risk_pct_snapshot"])
            except Exception as e:
                log(f"⚠️ Failed to attach risk snapshot for {symbol}: {e}")
                clean_info["target_risk_usd"] = 37.5
